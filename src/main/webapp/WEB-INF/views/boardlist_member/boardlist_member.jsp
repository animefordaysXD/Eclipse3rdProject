					...
					<div id="wrapper_list_inner">
						...
						<table class="list_table">
							...
							<tbody>
								...
								<c:choose>
									<c:when test="${empty boardlist_member}">
										<tr>
											<td colspan="7"><h2>자료가 존재하지 않습니다.</h2></td>
										</tr>
									</c:when>
									<c:otherwise>
										<c:forEach var="k" items="${boardlist_member}" varStatus="vs">
									...
									<td colspan="7">
										<ol class="paging">
											...
											<c:choose>
												...
												<c:otherwise>
													<li><a
														href="boardlist_member.do?cPage=${paging.beginBlock - paging.pagePerBlock}">이전으로</a></li>
												</c:otherwise>
											</c:choose>
											<!-- 블록안에 들어간 페이지번호들 -->
											<c:forEach begin="${paging.beginBlock}"
												...
												<c:choose>
													...
													<c:otherwise>
														<!-- 다른 페이지는 링크까지 -->
														<li><a href="boardlist_member.do?cPage=${k}">${k}</a></li>
													</c:otherwise>
												</c:choose>
											</c:forEach>
											<!-- 다음 -->
											<c:choose>
												...
												<c:otherwise>
													<li><a
														href="boardlist_member.do?cPage=${paging.beginBlock + paging.pagePerBlock}">다음으로</a></li>
												</c:otherwise>
											</c:choose>
										</ol>
									</td>
