...
<script type="text/javascript">
	function enterKey(e) {
		if (e.keyCode == 13) {
			...
			location.href = "boardlist_room_search.do?search=" + search
					+ "&radio=" + radio;
		}
	}
			...
			<div id="container_list">
				<div id="wrapper_list">
					<div id="wrapper_list_inner">
						<a href="boardlist_room.do" style="text-decoration: none;"><h2
								id="headline">방관리리스트</h2></a>
						<div id="container_radio">
							<form id="container_radio_form">
								<label class="radio_label"> <input type="radio"
									...
									name="radio" value="select_category"
									onclick="radio_select(event)" /> <span>카테고리</span>
								</label> <label class="radio_label"> <input type="radio"
									name="radio" value="select_room_title"
									onclick="radio_select(event)" /> <span>모임명</span>
								</label> <label class="radio_label"> <input type="radio"
									name="radio" value="select_u_nickname"
									onclick="radio_select(event)" /> <span>개설자</span>
								</label> <label class="radio_label"> <input type="radio"
									name="radio" value="select_write_datetime"
									onclick="radio_select(event)" /> <span>개설일</span>
								</label>
							</form>
						</div>
										...
										<c:forEach var="k" items="${boardlist_room}" varStatus="vs">
											<tr>
												...
												<td><a class="gradient-btn"
													href="boardlist_room_admindelete.do?room_idx=${k.room_idx}"
													onclick="room_delete()">모임방삭제</a></td>
											</tr>
										</c:forEach>
									</c:otherwise>
								</c:choose>
